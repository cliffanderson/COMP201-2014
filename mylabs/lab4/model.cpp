...
bool Model::valid(int row, int column) {
    return row < 8 && column < 8 && visible[row][column] == '_' && visible[row][column];
}
...
void Model::flip(int row, int column) {
	...
	//flipping the first cell in our attempt at a match
	...
	else if(state == FIRST)
	{
		...
		{
			//no match, set state to NO_MATCH and put data on vector
			state = NO_MATCH;
			
		}
		else
		{
			//we made a match, empty vectors and reset state
			state = INIT;
		}
	}
	else if(state == NO_MATCH)
	{
		//we don't have a match, but still want to show the user the cell
		//they flipped. In this state just reset the state to INIT
		//and flip the cells back over
